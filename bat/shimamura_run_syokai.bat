...
echo ==============================
echo  Shimamura Syokai Test
echo ==============================
echo.

:: ---- Scan shimamura profiles from env/ (excluding template) ----
set "IDX=0"
for /f "usebackq delims=" %%F in (`dir /b "%CD%\env\.env.shimamura.*" 2^>nul`) do (
  set "NAME=%%F"
  set "NAME=!NAME:.env.=!"
  if /i not "!NAME!"=="shimamura.template" (
    set /a IDX+=1
...
if %IDX%==0 (
  echo [NOTICE] env\.env.shimamura.* が見つかりません。
  echo   env\ フォルダに .env.shimamura.xxx ファイルを作成してください。
  pause
  exit /b 0
)
