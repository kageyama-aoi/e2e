...
module.exports = {
  ...
  /**
   * 候補生を「画面のフォームをそのまま送信する」方式で登録します（UI 操作を経由しない）。
   *
   * **用途**
   * 他テストの前提データ（候補生）をすばやく用意するためのもの。
   * 登録画面そのものの検証には使わない（検証は UI 経由の Scenario で行う）。
   *
   * **仕組み**
   ...
   * - `contact_status=5`（候補生）は送信内容に含まれず、サーバーが Referer / セッションで
   *   判定している。そのため必ず登録画面を開いた状態から送る（画面を開かずに送らない）。
   *
