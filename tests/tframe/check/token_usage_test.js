/**
 * @fileoverview トークンを利用したAPI連携テスト
 * 
 * **テスト内容**
 * - Beforeフックで管理者ログインと講師用トークンの取得を行う
 * - 取得したトークンを使用して「講師個人情報取得API」を実行する
 * - レスポンスの検証とログ出力を行う
 * 
 * **依存**
 * - apiCommonLoginPage: トークン取得用
 * - apiTeacherInfoGetPage: 個人情報取得API実行用
 * 
 * **作成日**
 * - 2025-12-20
 * 
...
Scenario('取得したトークンを後続の操作で利用する', async ({ I, apiTeacherInfoGetPage }) => {
  ...
  apiTeacherInfoGetPage.navigateToPersonalInfo();
  apiTeacherInfoGetPage.fetchInfoWithToken(tcnToken);

  I.say('Step 2: レスポンス内容を検証。');
  const responseSelector = apiTeacherInfoGetPage.locators.responseArea;
