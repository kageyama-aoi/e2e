...
/**
 * shimamura 候補生のフォーム送信登録（createContactViaApi）動作確認ランナー
 *
 * このファイルは通常のテストスイートには含まれない（`_test.js` 非末尾）。
 * `createContactViaApi()` が実環境で通るかを手動で確かめるための実行用で、
 * 実行すると testgcp に候補生が1件増える。
 *
 * 実行例:
 *   npx codeceptjs run ./tests/shimamura/util/create_contact_via_api_check.js --profile shimamura.testgcp
 *
 * 確認すること:
...
Feature('shimamura 候補生のフォーム送信登録（動作確認）');

Before(beforeShimamura);

Scenario('createContactViaApi で候補生を1件登録できる', async ({ I, contactRegisterPageShimamura }) => {
  ...
  const result = await contactRegisterPageShimamura.createContactViaApi({
  ...
  I.see(name.lastName);
  I.saveScreenshotWithTimestamp('CONTACT_CREATE_VIA_API', true);
});
